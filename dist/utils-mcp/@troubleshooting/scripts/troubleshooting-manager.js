...
exports.TroubleshootingManager = void 0;
const fs = __importStar(require("fs/promises"));
const path = __importStar(require("path"));
const events_1 = require("events");
const child_process_1 = require("child_process");
const util_1 = require("util");
const github_deployer_1 = require("./utils/github-deployer");
...
class TroubleshootingManager extends events_1.EventEmitter {
    constructor(targetDir) {
        ...
        // GitHub 배포 설정 초기화
        github_deployer_1.githubDeployer.updateConfig({
            branch: "main",
    ...
    async initializeTemplates() {
        try {
            ...
            // 템플릿 파일 저장
            await fs.writeFile(path.join(this.templatePath, "error-templates.json"), JSON.stringify(defaultTemplates, null, 2));
    ...
    async createNewTemplate(error, solution) {
    ...
    async saveTemplates() {
        try {
            ...
            await fs.writeFile(path.join(this.templatePath, "error-templates.json"), JSON.stringify(templateArray, null, 2));
        }
        catch (error) {
    ...
    async startWatching() {
        ...
        console.log(`🚀 GitHub 자동 배포: ${github_deployer_1.githubDeployer.isEnabled() ? "활성화" : "비활성화"}`);
    ...
    startFileWatcher() {
        ...
        this.fileWatcher = (0, fs_1.watch)(this.scriptsPath, { recursive: true }, async (eventType, filename) => {
            if (!filename)
                return;
                ...
                if (shouldAutoFix) {
                    ...
                    while (attempts < this.MAX_RECOVERY_ATTEMPTS && !fixed) {
                        ...
                        if (fixed) {
                            ...
                            // GitHub 자동 배포
                            const troubleshootingManualPath = path.join(this.manualPath, "troubleshooting.md");
                            await github_deployer_1.githubDeployer.deploy(`트러블슈팅: ${category.name} - ${error.message}`, fixedFiles, troubleshootingManualPath);
                            break;
                        }
                            ...
                            console.log(`❌ 최대 시도 횟수 초과, 수동 해결 필요: ${error.message}`);
                        }
                    }
                }
    ...
    findMatchingTemplate(error) {
        for (const template of this.templates.values()) {
            try {
                const regex = typeof template.pattern === "string"
    ...
    async attemptAutoFix(error, category, template) {
        try {
            if (template?.fixScript) {
                // 템플릿의 수정 스크립트 실행
                const fixScriptPath = path.join(this.templatePath, "fixes", template.fixScript + ".ts");
    ...
    checkDuplicate(error) {
        return this.errorHistory.some((item) => item.message === error.message && Date.now() - item.timestamp < 3600000 // 1시간 이내
        );
    }
    markErrorResolved(error, method) {
        const errorIndex = this.errorHistory.findIndex((item) => item.message === error.message);
        if (errorIndex !== -1) {
            this.errorHistory[errorIndex].resolved = true;
            this.errorHistory[errorIndex].resolution = method;
        }
    }
    async addToHistory(error, category) {
        const historyItem = {
            id: Date.now().toString(),
            timestamp: Date.now(),
            category: category.name,
    ...
    async stopWatching() {
        try {
            ...
            console.log(`- 해결된 에러: ${this.errorHistory.filter((e) => e.resolved).length}개`);
            console.log(`- 자동 복구된 에러: ${this.errorHistory.filter((e) => e.autoFixed).length}개`);
            console.log("=====================\n");
            await this.checkAndUpdateManual();
    ...
    async checkAndUpdateManual() {
        const recentErrors = this.errorHistory.filter((error) => Date.now() - error.timestamp < 3600000);
...
### 자동 복구 통계
- **성공률**: ${resolved.length > 0
            ? Math.round((autoFixed.length / resolved.length) * 100)
            : 0}%
- **평균 시도 횟수**: ${resolved.length > 0 ? Math.round(this.errorCount / resolved.length) : 0}회
`;
...
`;
    ...
    async processScripts() {
        try {
            ...
            const scriptFiles = files.filter((file) => file.endsWith(".ts") || file.endsWith(".js"));
    ...
    analyzeScript(content, filename) {
        const info = {
            filename,
            functions: [],
            dependencies: [],
        ...
        try {
            ...
            info.functions = functionMatches
                .map((m) => {
                const name = m.match(/function\s+(\w+)|const\s+(\w+)/)?.[1] ||
                    m.match(/const\s+(\w+)/)?.[1] ||
                    "";
                return name;
            })
            ...
            info.interfaces = interfaceMatches.map((m) => m.replace("interface ", ""));
            // export 항목 추출
            const exportMatches = content.match(/export\s+(?:class|interface|const|function|type)\s+(\w+)/g) || [];
            info.exports = exportMatches
                .map((m) => m.split(/\s+/).pop() || "")
                .filter(Boolean);
            // 의존성 추출
            const importMatches = content.match(/import\s+.*?from\s+['"](.+?)['"]/g) || [];
            info.dependencies = importMatches.map((m) => m.match(/from\s+['"](.+?)['"]/)?.[1] || "");
            // 설명 추출 (JSDoc 포함)
            ...
            info.description = descMatches
                ...
                .join("\n");
            // 에러 처리 패턴 추출
            const errorMatches = content.match(/catch\s*\((.*?)\)\s*{[\s\S]*?}/g) || [];
    ...
    async updateManualWithScript(scriptInfo) {
...
#### 구성요소
- **클래스**: ${scriptInfo.classes.length > 0
            ? scriptInfo.classes.map((c) => `\`${c}\``).join(", ")
            : "없음"}
- **인터페이스**: ${scriptInfo.interfaces.length > 0
            ? scriptInfo.interfaces.map((i) => `\`${i}\``).join(", ")
            : "없음"}
- **내보내기**: ${scriptInfo.exports.length > 0
...
`;
    ...
    updateGitHubConfig(config) {
        github_deployer_1.githubDeployer.updateConfig(config);
    }
    /**
     * GitHub 배포 활성화/비활성화
     */
    setGitHubDeploymentEnabled(enabled) {
        github_deployer_1.githubDeployer.setEnabled(enabled);
    }
    /**
     * GitHub 배포 이력 조회
     */
    getGitHubDeploymentHistory() {
        return github_deployer_1.githubDeployer.getDeploymentHistory();
    }
    startTimeoutTimer() {
...
exports.TroubleshootingManager = TroubleshootingManager;
//# sourceMappingURL=troubleshooting-manager.js.map